...
//    int[][] dirs = {{1,0},{-1,0},{0,1},{0,-1}};
    ...
    public List<String> findItinerary(List<List<String>> tickets) {
        ...
        for (List<String> t : tickets) {
            graph
                    .computeIfAbsent(t.get(0), k -> new PriorityQueue<>())
                    .add(t.get(1));
        }
        ...
        // Reverse because we add on backtrack
    ...
    void dfs(String airport) {
        ...
        while (pq != null && !pq.isEmpty()) {
            String next = pq.poll();   // consume edge
            dfs(next);
        }

        path.add(airport); // post-order
    }
    ...
    long calculate(TreeNode root, long totalSum) {
        if (root == null) return 0;
    ...
    public int countBattleships(char[][] board) {
        ...
        for (int i = 0; i < m; i++) {
            for (int j = 0; j < n; j++) {
                if (!InvalidShip(i, j, board)) Ships++;
            }
        }
        return Ships;
    }

    boolean InvalidShip(int i, int j, char[][] board) {
        for (int[] dir : dirs) {
            var ni = i + dir[0];
            var nj = j + dir[1];
            boolean boundary;
            if (ni < 0 || nj < 0 || ni == board.length || nj == board[0].length) boundary = false;
            else boundary = board[ni][nj] == 'X';
    ...
    private Boolean check(double limit_y, int[][] squares, double total_area) {
        ...
        for (int[] sq : squares) {
            ...
            int l = sq[2];
            if (y < limit_y) {
                area += (double) l * Math.min(limit_y - y, l);
            }
        }
    ...
    public int minPairSum(int[] nums) {
        ...
        while (i < j) {
            minimized_max = Math.max(minimized_max, nums[i++] + nums[j--]);
        }
        return minimized_max;
    }
    ...
    public long minimumCost(int[] nums, int k, int dist) {
        ...
        for (int i = 1; i < n; i++) {
            ...
            ans = Math.min(ans,
                    nums[0] + minKOrderedSum(nums, i, k - 1, dist));
        }
        return ans;
    }

    public long minKOrderedSum(int[] nums, int start, int k, int dist) {
        int n = nums.length;
        long sum = 0;
        ...
        for (int picked = 0; picked < k; picked++) {
            int end = Math.min(start + dist, n - (k - picked));

            while (ptr <= end) {
                minHeap.offer(new int[]{nums[ptr], ptr});
                ptr++;
            }

            while (!minHeap.isEmpty() && minHeap.peek()[1] < lastIndex) {
                minHeap.poll();
            }

            int[] chosen = minHeap.poll();
            sum += chosen[0];
            lastIndex = chosen[1] + 1;
        }
    ...
    public List<Boolean> camelMatch(String[] queries, String pattern) {
        ...
        for (String query : queries) list.add(ans(pattern, query));
        return list;
    }

    boolean ans(String p, String m) {
        // In this matching we are always thing to Test against Lower case addition
        // if we encounter a Big Letter that is not in a pattern with:
        // Order Based.
        // The same index and quantity that is FB -> will be false for FoFoBar, cuz F is 2 times but can't support as we can't add a new Big Case char . only small letter allowed
        var len = m.length();
        var len2 = p.length();
        int index = 0;
        ...
        for (int i = 0; i < len; i++) {
            ...
            if (x == sb.charAt(i)) index++;
            else {
                if (x - 'A' >= 0 && x - 'A' < 26) {
                    // Big letter isn't supported.
                    // if it was allowed, prev if check have checked it.
//                    System.out.println(sb);
                    return false;
                } else sb.insert(i, x); // add that char to the pattern.
            }
        }
//        System.out.println(sb);
        return index == len2;
    ...
    boolean optimise(String p, String m) {
        int i = 0; // pointer on pattern
        ...
        for (int j = 0; j < m.length(); j++) {
            ...
            if (i < p.length() && x == p.charAt(i)) i++; // consume pattern char
            else if (isUpper) return false; // extra uppercase not allowed
    ...
    public int videoStitching(int[][] clips, int time) {
        ...
        while (cur < time) {
            ...
            if (reach == cur) return -1; // Not possible reach
            ans++;
            cur = reach;
        }
        return ans;
    ...
    public int[] numMovesStones(int a, int b, int c) {
        ...
        int minMoves;
        if (gap1 == 1 && gap2 == 1) minMoves = 0;
        else if (gap1 <= 2 || gap2 <= 2) minMoves = 1;
        else minMoves = 2;
...
class StreamChecker {
    // we are getting words -> x y z
    // array = [xyz]
    // rather than checking from left to right  , we will check from r to l , this ensures Trie searching as from r to l ensures only if the given stream contains any form or not

    // we will Implement Trie searching to efficient searching in the string space
    ...
    static class Trie {
        ...
        void insert(String[] words) {
            for (String word : words) {
                ...
                Tnode node = root;
                int len = word.length();
                for (int i = len - 1; i >= 0; i--) { // reverse adding to check reverse
        ...
        boolean search(StringBuilder stream) {
            Tnode node = root;
    ...
    public boolean query(char letter) {
        ...
        if (stream.length() > trie.maxlen) stream.delete(0, stream.length() - trie.maxlen);
